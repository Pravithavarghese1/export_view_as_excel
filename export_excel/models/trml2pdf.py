...
from odoo.report.render.rml2pdf.utils import _regex
...
class _rml_styles(object,):
    def __init__(self, nodes, localcontext):
        ...
        self.names = {}
        self.table_styles = {}
        self.default_style = lib.styles.getSampleStyleSheet()
        ...
        for node in nodes:
            ...
            for style in node.findall('paraStyle'):
                ...
                self.styles_obj[sname] = lib.styles.ParagraphStyle(sname, self.default_style["Normal"], **self.styles[sname])
    ...
    def _para_style_update(self, node):
        ...
        if node.get('alignment'):
            align = {
                'right':lib.enums.TA_RIGHT,
                'center':lib.enums.TA_CENTER,
                'justify':lib.enums.TA_JUSTIFY
            }
            data['alignment'] = align.get(node.get('alignment').lower(), lib.enums.TA_LEFT)
        return data
...
class _rml_flowable(object):
    ...
    def _flowable(self, node, extra_style=None):
        ...
        if node.tag=='para':
            ...
            styles = lib.styles.getSampleStyleSheet()
            style = styles['Title']
